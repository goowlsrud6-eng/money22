...
st.set_page_config(page_title="자금 관리 v95", layout="wide", page_icon="💰")
run_backup()

@st.cache_resource
def get_db_connection():
    conn = sqlite3.connect('finance_final_v95.db', check_same_thread=False)
    c = conn.cursor()
    # 거래처 마스터
...
def process_ecount_v95(file):
    try:
        ...
        total = to_float(df.iloc[df.iloc[:, 5].last_valid_index(), 5]) if curr != "한화" else to_float(str(df.iloc[4, 0]).split(":")[-1])
...
# [Tab 1] 입금 엑셀 업로드 (거래처 비워도 발주번호로 연동되는 로직 포함)
with tabs[1]:
    st.header("📂 통합 입금 엑셀 업로드")
    sample = pd.DataFrame(columns=["발주번호", "거래처", "유형", "상품명", "입금일", "실입금액", "선급금액", "송금사유"])
    st.download_button("📥 샘플 양식 다운로드", sample.to_csv(index=False).encode('utf-8-sig'), "payment_template.csv")
...
with tabs[2]:
    ...
    if not o_data.empty:
        st.subheader("📄 발주 리스트 (마감 및 소급 동기화)")
        show_all = st.checkbox("마감된 발주 포함해서 보기", value=False)
        disp_o = o_data if show_all else o_data[o_data['마감여부'] == 0]
        ev_o = st.data_editor(disp_o[['발주번호', '발주차수', '거래처명', '상품명', '유형', '통화', '발주총액', '마감여부', '발주일']].sort_values('발주일', ascending=False), 
                              ...
                              column_config={"마감여부": st.column_config.CheckboxColumn("마감")})
...
# [Tab 3] 상세내역 및 통합 정산 (월별 필터 + 검색 + 정산현황)
with tabs[3]:
    ...
    if not p_all.empty:
        ...
        search = c3.text_input("업체명 또는 상품명 검색")
        ...
        if c_del1.button("🗑️ 삭제"): conn.execute(f"DELETE FROM payments WHERE id={did}"); conn.commit(); st.rerun()
...
# [Tab 5] 환율 관리
with tabs[5]:
    st.header("📈 월별 평균 환율 관리")
    st.info("매월 초 지난달의 일별 환율을 입력해 두면 월평균 환율을 자동 계산합니다.")
    c_ex1, c_ex2 = st.columns([1, 1.5])
    with c_ex1:
        st.subheader("🗓️ 일별 환율 입력")
        with st.form("ex_rate_form", clear_on_submit=True):
            ed = st.date_input("날짜")
            eu = st.number_input("USD", format="%.2f", value=1350.0)
            ec = st.number_input("CNY", format="%.2f", value=190.0)
            if st.form_submit_button("➕ 등록"):
                conn.execute("INSERT OR REPLACE INTO exchange_rates VALUES (?,?,?)", (ed.strftime("%Y-%m-%d"), eu, ec))
                conn.commit(); st.rerun()
        ex_raw = pd.read_sql("SELECT * FROM exchange_rates ORDER BY 날짜 DESC", conn)
        if not ex_raw.empty:
            ed_ex = st.data_editor(ex_raw, hide_index=True, use_container_width=True)
            if st.button("💾 환율 수정"):
                for _, r in ed_ex.iterrows(): conn.execute("UPDATE exchange_rates SET usd=?, cny=? WHERE 날짜=?", (r['usd'], r['cny'], r['날짜']))
                conn.commit(); st.rerun()
    with c_ex2:
        st.subheader("📊 월별 평균 환율 통계")
        if not ex_raw.empty:
            ex_raw['연월'] = ex_raw['날짜'].apply(lambda x: x[:7])
            monthly_avg = ex_raw.groupby('연월').agg({'usd':'mean', 'cny':'mean'}).reset_index()
            st.table(monthly_avg.style.format({'usd':'{:,.2f}', 'cny':'{:,.2f}'}))
            st.line_chart(ex_raw.set_index('날짜')[['usd', 'cny']])