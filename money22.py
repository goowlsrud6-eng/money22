...
def run_backup():
    ...
    db_file = 'finance_v18_final.db'  # 충돌 방지를 위해 새 DB 파일명 사용
...
# 2. 페이지 설정
st.set_page_config(page_title="자금 관리 시스템 v18", layout="wide", page_icon="💰")
run_backup()

# 3. DB 연결 및 초기화
@st.cache_resource
def get_db_connection():
    conn = sqlite3.connect('finance_v18_final.db', check_same_thread=False)
    c = conn.cursor()
    # 거래처 마스터
...
# --- Tab 3: 발주서 등록 (이카운트 업로드 + 수기) ---
with tabs[2]:
    st.header("📥 발주서 등록 (ERP 연동 및 수기)")
    ...
    with col_o1:
        ...
        if ecount_file:
            st.info("파일을 분석하여 아래 수기 폼에 내용을 채웁니다 (준비중: 현재는 수기입력 권장)")
            # 파싱 로직은 이전에 논의한 위치값에 따라 여기에 추가 가능

    with col_o2:
        st.subheader("✍️ 발주서 수기 등록")
        with st.form("order_reg_manual"):
            o_id = st.text_input("발주번호 (전표번호)")
            o_date = st.date_input("발주일", datetime.now())
            o_cat = st.selectbox("유형", CATEGORIES)
            o_vendor = st.selectbox("거래처 선택", ["선택"] + list(v_list['name']) if not v_list.empty else ["업체 없음"])
            o_prod = st.text_input("대표 상품명")
...
with tabs[0]:
    ...
    v_data = load_data("vendors")
    active_orders = orders[orders['is_closed'] == 0] if not orders.empty else pd.DataFrame()
    ...
    with st.form("pay_input_form"):
        ...
        p_vendor = c2.selectbox("🏢 업체명", ["선택"] + list(v_data['name']) if not v_data.empty else ["선택"])
        p_cat = c3.selectbox("📁 유형", CATEGORIES)
        
        # 계좌 정보 안내
        bank, acc, hold = "", "", ""
        if p_vendor != "선택" and not v_data.empty:
