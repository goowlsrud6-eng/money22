...
def run_backup():
    ...
    db_file = 'finance_v38_final.db'
    backup_file = f"backups/backup_{today_str}.db"
    if os.path.exists(db_file) and not os.path.exists(backup_file):
        shutil.copy2(db_file, backup_file)

# 2. 페이지 설정 및 DB 연결
st.set_page_config(page_title="자금 관리 v38", layout="wide", page_icon="💰")
run_backup()

@st.cache_resource
def get_db_connection():
    conn = sqlite3.connect('finance_v38_final.db', check_same_thread=False)
    c = conn.cursor()
...
# --- 3. 이카운트 정밀 분석 함수 (한화 B7 / 외화 C7 / 대괄호 제거) ---
def process_ecount_v38(file):
    try:
        ...
        if currency == "한화":
            prod_list = df_raw.iloc[6:, 1].dropna().astype(str).tolist() # B7 시작
        else:
            prod_list = df_raw.iloc[6:, 2].dropna().astype(str).tolist() # C7 시작
...
with tabs[0]:
    ...
    with st.form("pay_manual_v38", clear_on_submit=True):
        ...
        p_dep = c4.number_input("실입금액", min_value=0.0)
        p_pre = c5.number_input("선급금액", min_value=0.0)
...
with tabs[1]:
    ...
    p_file = st.file_uploader("입금 내역 CSV", type=['csv'], key="p_csv_v38")
...
with tabs[2]:
    ...
    with col1:
        ...
        o_file = st.file_uploader("이카운트 발주서(.xlsx) 업로드", type=['xlsx'], key="o_up_v38")
        if o_file:
            success, res = process_ecount_v38(o_file)
            if success: st.success(f"✅ 등록 완료: {res}")
            else: st.error(f"❌ 분석 실패: {res}")
    with col2:
        st.subheader("✍️ 2. 발주서 수기 등록")
        v_list = load_table("vendors")
        with st.form("o_manual_v38", clear_on_submit=True):
...
with tabs[4]:
    ...
    with col_v1:
        st.subheader("➕ 개별 등록")
        with st.form("v_reg_v38", clear_on_submit=True):
    ...
    with col_v2:
        ...
        v_file = st.file_uploader("거래처 CSV 업로드", type=['csv'], key="v_csv_v38")
