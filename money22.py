...
st.set_page_config(page_title="자금 관리 v99", layout="wide", page_icon="💰")
run_backup()

@st.cache_resource
def get_db_connection():
    conn = sqlite3.connect('finance_final_v99.db', check_same_thread=False)
    c = conn.cursor()
...
with tabs[1]:
    ...
    if f_p and st.button("🚀 데이터 일괄 저장"):
        ...
        conn.commit(); st.session_state.pay_up_key += 1; st.rerun()
...
with tabs[2]:
    ...
    c1, c2 = st.columns(2)
    with c1:
        of_list = st.file_uploader("발주서(xlsx)", type=['xlsx'], accept_multiple_files=True, key=f"ord_{st.session_state.order_up_key}")
        if of_list and st.button("🚀 일괄 등록"):
            for of in of_list: process_ecount_v99(of)
            st.session_state.order_up_key += 1; st.rerun()
    st.divider()
    o_data = pd.read_sql("SELECT * FROM orders", conn)
    if not o_data.empty:
        show_c = st.checkbox("마감 포함 보기", value=False)
...
with tabs[3]:
    ...
    if not p_all.empty:
        ...
        if not fil_p.empty:
            ...
            cat_sum['합계'] = cat_sum['실입금액'] + cat_sum['선급금액']
...
# [Tab 5] 환율 관리 (사용자 요청: 차트 분리 및 병렬 비교 강화)
with tabs[5]:
    st.header("📈 환율 정밀 분석 (Investing.com 연동)")
    st.info("USD(1400원대)와 CNY(200원대)를 분리하여 가시성을 높였습니다. 연도별 병렬 비교표를 통해 전년/전월 동월 대비 변화를 확인하세요.")
    ...
    if not ex_db.empty:
        ...
        # --- 2. 분리된 시각화 차트 ---
        st.subheader("📉 월별 평균 환율 추이 (차트 분리)")
        ...
        # --- 3. 병렬 비교 분석표 생성 함수 (YoY / MoM 포함) ---
        def get_enhanced_comparison_table(df, col_name):
            years = sorted(df['연도'].unique(), reverse=True)
            if len(years) < 1: return pd.DataFrame()
            ...
            final_view = pd.DataFrame(index=pivot_df.index)
            
            # 전년도(24년) 평균
            if prev_y:
                final_view[f'{prev_y}년 평균'] = pivot_df[prev_y]
            
            # 올해(25년) 평균
            final_view[f'{curr_y}년 평균'] = pivot_df[curr_y]
            
            # 전년 동월 대비 (YoY)
            if prev_y:
                final_view['전년비(YoY)'] = final_view[f'{curr_y}년 평균'] - final_view[f'{prev_y}년 평균']
            
            # 올해 내 전월 대비 (MoM)
            final_view['전월비(MoM)'] = final_view[f'{curr_y}년 평균'].diff()
            
            return final_view.reset_index()
        ...
        with rep_col1:
            st.write("#### 💵 USD 분석 (달러)")
            usd_rep = get_enhanced_comparison_table(monthly_mean, 'usd')
            if not usd_rep.empty:
                st.table(usd_rep.style.format({'월': '{:.0f}월', f'{years[0]}년 평균': '{:,.2f}', 
                                               f'{years[1]}년 평균' if len(years)>1 else 'X': '{:,.2f}',
                                               '전년비(YoY)': '{:+.2f}', '전월비(MoM)': '{:+.2f}'}))
        with rep_col2:
            st.write("#### 💴 CNY 분석 (위안)")
            cny_rep = get_enhanced_comparison_table(monthly_mean, 'cny')
            if not cny_rep.empty:
                st.table(cny_rep.style.format({'월': '{:.0f}월', f'{years[0]}년 평균': '{:,.2f}', 
                                               f'{years[1]}년 평균' if len(years)>1 else 'X': '{:,.2f}',
                                               '전년비(YoY)': '{:+.2f}', '전월비(MoM)': '{:+.2f}'}))
    else:
        st.warning("환율 데이터가 없습니다. 상단 CSV 업로드 기능을 이용해 주세요.")