...
st.set_page_config(page_title="자금 관리 v50", layout="wide", page_icon="💰")
run_backup()

@st.cache_resource
def get_db_connection():
    conn = sqlite3.connect('finance_v50_final.db', check_same_thread=False)
    c = conn.cursor()
...
with tabs[0]: # 수기 입력
    ...
    with st.form("p_manual_v50", clear_on_submit=True):
...
with tabs[1]: # 입금 엑셀
    ...
    st.download_button("📥 입금 양식 다운로드", p_tmp.to_csv(index=False).encode('utf-8-sig'), "payment_temp_v50.csv")
    f_p = st.file_uploader("입금 CSV", type=['csv'], key="p_csv_v50")
    if f_p and st.button("🚀 입금 데이터 저장"):
        try:
            df_p = pd.read_csv(f_p); o_df = pd.read_sql("SELECT * FROM orders", conn); v_df = pd.read_sql("SELECT * FROM vendors", conn)
            for _, r in df_p.iterrows():
                pd_s = smart_date(r['입금일']); oid = str(r['발주번호']).strip() if pd.notna(r['발주번호']) else None
                ...
                vi = v_df[v_df['거래처명'] == vn] # 여기서 오타 수정됨
                ...
                tot = float(r['실입금액']) + float(r['선급금액'])
                conn.cursor().execute("INSERT INTO payments (발주번호, 입금일, 유형, 거래처명, 상품명, 통화, 실입금액, 선급금액, 메모, 한화환산액, 은행, 계좌번호, 예금주) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
                                     (oid, pd_s, pc, vn, pp, cur, r['실입금액'], r['선급금액'], r['송금사유'], tot*rt, bk, ac, hd))
...
with tabs[2]: # 발주서 등록
    ...
    c1, c2 = st.columns(2)
    with c1:
        of = st.file_uploader("엑셀 업로드", type=['xlsx'], key="of_v50")
        if of: s, r = process_ecount_v50(of); st.success(f"성공: {r}") if s else st.error(f"오류: {r}")
    with c2:
        with st.form("o_manual_v50", clear_on_submit=True):
            ...
            if st.form_submit_button("수기 저장"):
                if mi and mv != "선택": conn.execute("INSERT OR REPLACE INTO orders VALUES (?,?,?,?,?,?,?,0)", (mi, md.strftime("%Y-%m-%d"), mv, "수기등록", "사입", "한화", 0)); conn.commit(); st.rerun()
    o_l = pd.read_sql("SELECT * FROM orders", conn)
    if not o_l.empty:
        od = o_l.copy(); od['발주총액'] = od['발주총액'].apply(format_num)
...
with tabs[3]: # 상세내역
    ...
    if not p_df.empty:
        ...
        st.dataframe(df_f.sort_values('입금일', ascending=False).style.apply(style_row, axis=1), use_container_width=True, hide_index=True)
        st.divider()
...
with tabs[4]: # 거래처 관리 (KeyError 해결 버전)
    st.header("⚙️ 거래처 관리")
    cv1, cv2 = st.columns(2)
    with cv1:
        st.subheader("➕ 개별 등록")
        with st.form("v_reg_v50", clear_on_submit=True):
    ...
    with cv2:
        ...
        st.download_button("📥 거래처 양식 다운로드", v_tmp.to_csv(index=False).encode('utf-8-sig'), "vendor_template_v50.csv")
        vf = st.file_uploader("거래처 CSV 업로드", type=['csv'], key="v_csv_v50")
        if vf and st.button("🚀 일괄 업로드"):
            v_df_up = pd.read_csv(vf)
            # 인덱스가 아닌 '컬럼명'으로 안전하게 매칭합니다.
