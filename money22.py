...
# --- [Tab 1] 발주서 등록 및 관리 (천 단위 쉼표 & 행 전체 음영 적용) ---
with tabs[1]:
    ...
    with c1:
        ...
        # [원본 기능] 이카운트 엑셀 일괄 업로드 [cite: 37, 38]
        ...
        if o_files and st.button("🚀 발주서 일괄 분석 실행", use_container_width=True):
            for f in o_files: 
                success, msg = process_ecount_v136_cloud(f) # 클라우드용 분석 함수 호출
                if not success: st.error(f"[{f.name}] {msg}")
            st.session_state.order_up_key += 1; st.success("분석 완료!"); st.rerun()
        
        st.divider()
        # [원본 기능] 수기 직접 입력 폼 (발주차수 포함) [cite: 39, 40]
        with st.form("manual_ord_form_v136_final", clear_on_submit=True):
            st.write("**📝 직접 발주 입력**")
            m_oid = st.text_input("발주번호 (필수)")
            m_step = st.text_input("발주차수 (예: 초도, 리오더 1차)") [cite: 39]
            
            vn_list = ["선택"] + list(v_master['거래처명'].unique()) if not v_master.empty else ["선택"]
            m_vn = st.selectbox("거래처 선택", vn_list) [cite: 40]
            
            m_prod = st.text_input("상품명") [cite: 40]
            
            col_m1, col_m2 = st.columns(2)
            m_amt = col_m1.number_input("발주총액", format="%.2f") [cite: 40]
            m_cur = col_m2.selectbox("통화", ["한화", "USD", "CNY"]) [cite: 40]
            
            if st.form_submit_button("➕ 발주 저장", use_container_width=True):
                if m_oid and m_vn != "선택":
                    # 거래처 마스터에서 기본 유형 자동 추출
                    ...
                    upsert_supabase_data("orders", new_order) [cite: 42]
                    st.success(f"발주번호 {m_oid} 등록 완료!"); st.rerun()
                else:
                    st.error("발주번호와 거래처는 필수 항목입니다.")

    with c2:
        st.subheader("2. 발주 목록 및 마감 관리")
        
        if not o_data.empty:
            # [원본 기능] 마감 건 숨기기/보여주기 체크박스 
            show_all = st.checkbox("이미 마감된 발주서까지 모두 보기", value=True)
            
            disp_o = o_data.copy()
            if not show_all:
                disp_o = disp_o[disp_o['마감여부'] == 0] [cite: 43]
            
            # [가시성 보강] 마감 건 행 전체 음영 및 흐림 처리 스타일 함수 [cite: 56, 57]
            def style_orders_v136(row):
                if row['마감여부'] == 1:
                    # 행 전체 배경색 회색, 글자색 연회색 처리
                    return ['background-color: #f0f2f6; color: #a1a1a1; border-color: #e6e9ef'] * len(row) [cite: 57]
                return [''] * len(row)

            # 최신 발주일 순으로 정렬
            disp_o = disp_o.sort_values('발주일', ascending=False)
            
            # 데이터 편집기 (천 단위 쉼표 포맷 적용)
            ev_o = st.data_editor(
                disp_o.style.apply(style_orders_v136, axis=1), # 행 단위(axis=1) 스타일 적용
                hide_index=True, 
                use_container_width=True,
                key=f"ord_editor_v136_{len(disp_o)}",
                column_config={
                    "마감여부": st.column_config.CheckboxColumn("마감"), [cite: 44]
                    "발주총액": st.column_config.NumberColumn("총액", format="%,.2f"), # 천 단위 쉼표 보정
                    "발주차수": st.column_config.TextColumn("차수")
                },
                disabled=["발주번호"] [cite: 43]
            )
            
            # [원본 로직] 수정사항 저장 및 Payments 테이블 소급 적용 [cite: 44, 45, 46]
            if st.button("💾 수정 내용 저장 및 입금내역 소급 적용", use_container_width=True):
                # 1. Orders 테이블 업데이트
                upsert_supabase_data("orders", ev_o.to_dict(orient='records')) [cite: 45]
                
                # 2. Payments 테이블 소급 업데이트 (차수 포함 모든 정보 동기화)
                for _, r in ev_o.iterrows():
                    supabase.table("payments").update({
                        ...
                        "유형": r['유형'],
                        "발주차수": r['발주차수']
                    }).eq("발주번호", r['발주번호']).execute() [cite: 46]
                
                st.success("발주 정보 및 연동된 입금 내역이 모두 업데이트되었습니다."); st.rerun()
        else:
            st.info("등록된 발주 내역이 없습니다. 왼쪽에서 먼저 등록해 주세요.")
            
# --- [Tab 2] 상세 내역 및 통합 정산 (최종 수정본) ---
with tabs[2]:
    ...
    if not p_all.empty:
        # [원본 로직] 발주번호 기준 정보(거래처/상품명/유형/발주차수) 자동 매칭 
        if not o_all.empty:
        ...
        # 엑셀 '송금사유' 매핑 및 날짜 안정화 [cite: 35]
        if '송금사유' in p_all.columns: 
            p_all['메모'] = p_all['메모'].fillna(p_all['송금사유'])
        
        p_all['dt'] = pd.to_datetime(p_all['입금일'], errors='coerce')
        p_all = p_all.dropna(subset=['dt'])
        
        # 2. 필터 섹션 (원본 [수정 4] 연도 범위 조회 기능 복구) [cite: 48, 49, 50]
        col_fil, col_sum_ui = st.columns([1, 1.2])
        
        with col_fil:
            st.subheader("🔍 조회 조건 설정")
            all_years = sorted(p_all['dt'].dt.year.unique())
            ...
            target_month = st.selectbox("조회 월 (전체 선택 시 연간 조회)", ["전체"] + sorted(list(p_all['dt'].dt.month.unique())))
            ...
            if target_month != "전체": 
                filtered = filtered[filtered['dt'].dt.month == int(target_month)]
            if filter_cat != "전체 유형": 
        ...
        with col_sum_ui:
            ...
            if not filtered.empty:
                summary = filtered.groupby('유형').agg({'실입금액':'sum', '선급금액':'sum', '한화환산액':'sum'}).reset_index()
                st.table(summary.style.format({'실입금액':'{:,.2f}', '선급금액':'{:,.2f}', '한화환산액':'{:,.0f}'}))

        st.divider()

        # 4. 발주번호별 정산 및 미수금 현황 (AttributeError 해결 버전) [cite: 54, 55, 56]
        ...
        settle_df['미수잔액'] = settle_df['발주총액'] - (settle_df['실입금액'] + settle_df['선급금액'])
        settle_df['진행상태'] = settle_df['마감여부'].apply(lambda x: "✅ 마감" if x == 1 else "⏳ 진행")
        
        # [수정] AttributeError 방지를 위해 map 방식 스타일 적용
        def apply_settle_styles(df):
            # 기본 스타일 설정
            styles = pd.DataFrame('', index=df.index, columns=df.columns)
            
            # 마감 건 회색 처리
            mask_closed = df['진행상태'] == '✅ 마감'
            styles.loc[mask_closed, :] = 'background-color: #f9f9f9; color: #bbbbbb'
            
            # 선급금 빨간색 (진행 중인 경우만)
            mask_pre = (df['선급금액'] > 0) & (~mask_closed)
            styles.loc[mask_pre, '선급금액'] = 'color: red; font-weight: bold'
            
            # 미수잔액 파란색 (진행 중인 경우만)
            mask_res = (df['미수잔액'] > 0) & (~mask_closed)
            styles.loc[mask_res, '미수잔액'] = 'color: blue; font-weight: bold'
            
            return styles

        display_cols = settle_df[['발주번호','발주차수','진행상태','거래처명','상품명','발주총액','실입금액','선급금액','미수잔액','통화']].sort_values(['진행상태', '발주번호'], ascending=[False, False])
        
        st.dataframe(
            display_cols.style.apply(apply_settle_styles, axis=None).format({
                '발주총액': '{:,.2f}', '실입금액': '{:,.2f}', '선급금액': '{:,.2f}', '미수잔액': '{:,.2f}'
            }),
        ...
        # 5. 상세 리스트 편집 및 삭제 [cite: 59, 61, 62, 65]
        st.subheader("📝 상세 내역 수정 및 삭제 관리")
        filtered['삭제'] = False
        ...
        if not filtered.empty:
            ...
            edited_p = st.data_editor(
                display_p, 
                hide_index=True, use_container_width=True, 
                key=f"p_edit_v_full_v136_{len(display_p)}",
                column_config={
                    "삭제": st.column_config.CheckboxColumn("삭제?"),
                    "상품명": st.column_config.TextColumn("상품명", width="large"),
                    "메모": st.column_config.TextColumn("메모", width="large"),
                    "한화환산액": st.column_config.NumberColumn("환산액", format="%d", disabled=True),
                    "입금일": st.column_config.TextColumn("입금일")
                }
            )
            
            c_btn1, c_btn2 = st.columns(2)
            if c_btn1.button("💾 상세 내역 수정 저장", key="save_final_p", use_container_width=True):
            ...
            if c_btn2.button("🗑️ 선택 항목 삭제 실행", key="del_final_p", use_container_width=True):
                to_del = edited_p[edited_p['삭제'] == True]
                for tid in to_del['id']:
                    supabase.table("payments").delete().eq("id", tid).execute()
                st.warning(f"{len(to_del)}건 삭제 완료"); st.rerun()

        # 6. 하단 메트릭 요약 [cite: 65]
        st.divider()
        m1, m2, m3, m4 = st.columns(4)
        m1.metric("조회범위 총 환산액", f"{filtered['한화환산액'].sum():,.0f} 원")
        m2.metric("KRW 합계", f"{filtered[filtered['통화']=='한화']['실입금액'].sum():,.0f}")
        m3.metric("USD 합계", f"${filtered[filtered['통화']=='USD']['실입금액'].sum():,.2f}")
        m4.metric("CNY 합계", f"¥{filtered[filtered['통화']=='CNY']['실입금액'].sum():,.2f}")
    else:
        st.info("데이터가 없습니다.")
