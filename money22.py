...
# 2. 페이지 설정 및 DB 연결
st.set_page_config(page_title="자금 관리 v56", layout="wide", page_icon="💰")
run_backup()

@st.cache_resource
def get_db_connection():
    conn = sqlite3.connect('finance_v56_final.db', check_same_thread=False)
    c = conn.cursor()
    # 거래처 마스터
...
# --- 4. 이카운트 분석 함수 (거래처 유형 연동 강화) ---
def process_ecount_v56(file):
    try:
        ...
        # ★ 핵심 수정: DB에서 해당 거래처의 기본유형을 먼저 가져옴 ★
...
with tabs[0]:
    ...
    with st.form("p_manual_v56", clear_on_submit=True):
...
with tabs[1]:
    ...
    st.download_button("📥 입금 양식 다운로드", p_tmp.to_csv(index=False).encode('utf-8-sig'), "payment_temp_v56.csv")
    f_p = st.file_uploader("입금 CSV 선택", type=['csv'], key="p_csv_v56")
...
with tabs[2]:
    ...
    with c1:
        ...
        of_list = st.file_uploader("발주서(.xlsx)들을 한 번에 선택하세요", type=['xlsx'], key="of_v56", accept_multiple_files=True)
        if of_list and st.button("🚀 모든 파일 등록"):
            for of in of_list: process_ecount_v56(of)
            st.success("일괄 등록 완료!"); st.rerun()
    with c2:
        st.subheader("✍️ 2. 발주서 수기 등록")
        with st.form("o_manual_v56", clear_on_submit=True):
...
# --- Tab 4: 상세내역 및 유형별 통합 정산 (핵심 업데이트) ---
with tabs[3]:
    st.header("🔍 상세 내역 및 통합 정산 (더블클릭 수정 가능)")
    p_df = pd.read_sql("SELECT * FROM payments", conn); o_df = pd.read_sql("SELECT * FROM orders", conn)
    
    if not p_df.empty:
        # 1. 유형별 요약 (물류비, 원단비 등)
        st.subheader("📋 유형별 지출 요약 (발주서 없는 비용 포함)")
        cat_sum = p_df.groupby('유형').agg({'실입금액':'sum', '선급금액':'sum'}).reset_index()
        cat_sum['전체지출'] = cat_sum['실입금액'] + cat_sum['선급금액']
        st.table(cat_sum.style.format({'실입금액': '{:,.2f}', '선급금액': '{:,.2f}', '전체지출': '{:,.2f}'}))
        ...
        edited_p = st.data_editor(p_df, hide_index=True, use_container_width=True, disabled=["id"], key="p_editor_v56")
        ...
        c1, c2 = st.columns(2)
        with c1:
            did = st.number_input("삭제할 ID 입력", min_value=0, step=1)
            if st.button("🗑️ 선택 행 완전 삭제"):
...
# --- Tab 5: 거래처 관리 (편집 기능 추가) ---
with tabs[4]:
    st.header("⚙️ 거래처 관리 (더블클릭 수정)")
    v_df = pd.read_sql("SELECT * FROM vendors", conn)
    if not v_df.empty:
        edited_v = st.data_editor(v_df, hide_index=True, use_container_width=True, key="v_editor_v56")
        if st.button("💾 거래처 정보 업데이트"):
            ...
            conn.commit(); st.success("거래처 정보가 수정되었습니다!"); st.rerun()
    
    st.divider()
    with st.form("v_reg_v56", clear_on_submit=True):
