...
# ==========================================
# 1. 시스템 설정 및 데이터베이스 초기화 (무삭제)
# ==========================================
def run_backup():
    """데이터 보존을 위한 자동 백업 시스템"""
    if not os.path.exists('backups'):
        os.makedirs('backups')
    db_file = 'finance_final_v113.db'
    today_str = datetime.now().strftime('%Y%m%d')
    backup_file = f"backups/backup_{today_str}.db"
    if os.path.exists(db_file) and not os.path.exists(backup_file):
        shutil.copy2(db_file, backup_file)

st.set_page_config(page_title="자금 관리 시스템 v113", layout="wide", page_icon="💰")
run_backup()

@st.cache_resource
def get_db_connection():
    """데이터베이스 테이블 스키마 정의 (단 한 줄도 생략 없음)"""
    conn = sqlite3.connect('finance_final_v113.db', check_same_thread=False)
    c = conn.cursor()
    # [1] 거래처 마스터
...
# ==========================================
# 2. 핵심 분석 및 유틸리티 엔진 (Fix 유지)
# ==========================================
def to_float(val):
    """쉼표 제거 및 숫자 변환 (Investing.com 대응)"""
    try:
...
def smart_date(date_str):
    """날짜 형식 통일 (Investing.com의 '2026- 03- 31' 형식 완벽 대응)"""
    try:
...
def process_exchange_csv(file, currency_type):
    """Investing.com CSV를 읽어 DB 업데이트 (컬럼명 유연성 확보)"""
    try:
        df = pd.read_csv(file)
        # 컬럼명에서 공백 및 BOM 제거
...
def process_ecount_v113(file):
    """ERP 발주서 분석 엔진 (전통적 로직 무삭제 유지)"""
    try:
...
with tabs[0]:
    ...
    with st.form("manual_pay_form_v113", clear_on_submit=True):
        c1, c2 = st.columns(2)
        p_oid = c1.selectbox("🔗 진행중인 발주번호 연동", ["없음"] + list(o_active['발주번호']))
...
# ------------------------------------------
# [Tab 1] 입금 엑셀 업로드 (양식 버튼 복구)
# ------------------------------------------
with tabs[1]:
    st.header("📂 통합 입금 엑셀 업로드")
    # [입금 샘플 양식]
    pay_tmp = pd.DataFrame(columns=["발주번호", "거래처", "유형", "상품명", "입금일", "실입금액", "선급금액", "송금사유"])
    st.download_button(label="📥 입금 업로드 샘플 양식 다운로드", data=pay_tmp.to_csv(index=False).encode('utf-8-sig'), file_name='payment_template.csv')
...
# ------------------------------------------
# [Tab 2] 발주서 등록 및 마감 (양식 + 수기폼 + 소급 복구)
# ------------------------------------------
with tabs[2]:
    ...
    with col_o1:
        ...
        st.download_button(label="📥 수기용 발주 양식 다운로드", data=ord_tmp.to_csv(index=False).encode('utf-8-sig'), file_name='order_manual_template.csv')
        ...
        if of_list and st.button("🚀 모든 발주서 일괄 등록"):
            for of in of_list: process_ecount_v113(of)
    ...
    with col_o2:
        ...
        with st.form("manual_order_form_v113"):
...
with tabs[4]:
    ...
    with cv1:
        ...
        with st.form("vendor_reg_form_v113", clear_on_submit=True):
            vn = st.text_input("거래처명"), vt = st.selectbox("유형", CATEGORIES)
            vc1, vc2, vc3 = st.columns(3)
            vb, va, vh = vc1.text_input("은행"), vc2.text_input("계좌"), vc3.text_input("예금주")
            if st.form_submit_button("✅ 거래처 저장"):
                if vn: conn.execute("INSERT OR REPLACE INTO vendors VALUES (?,?,?,?,?)", (vn[0],vb,va,vh,vt)); conn.commit(); st.success("완료!"); st.rerun()
    with cv2:
...
# ------------------------------------------
# [Tab 5] 환율 관리 (리포트 버그 완전 해결판)
# ------------------------------------------
with tabs[5]:
    st.header("📈 환율 정밀 분석 (Investing.com 연동)")
    cu1, cu2 = st.columns(2)
    with cu1:
        f_usd = st.file_uploader("USD/KRW CSV 업로드", type=['csv'], key="u_up")
    ...
    if not ex_db.empty:
        ...
        with cc1:
            fig_u = go.Figure(); fig_u.add_trace(go.Scatter(x=m_mean['ym'], y=m_mean['usd'], mode='lines+markers', name='USD'))
            fig_u.update_layout(yaxis=dict(range=[1360, 1540], dtick=20), height=350, template="plotly_white")
            st.plotly_chart(fig_u, use_container_width=True)
        with cc2:
            fig_c = go.Figure(); fig_c.add_trace(go.Scatter(x=m_mean['ym'], y=m_mean['cny'], mode='lines+markers', name='CNY', line=dict(color='orange')))
            fig_c.update_layout(yaxis=dict(range=[186, 226], dtick=2), height=350, template="plotly_white")
            st.plotly_chart(fig_c, use_container_width=True)

        # 리포트 생성 (가장 최신 연도 기준)
        def build_perfect_report(df, col):
            ...
            # 올해 데이터 매핑
            curr_data = df[df['year'] == curr_y].set_index('month')[col]
            res[f'{curr_y}년 평균'] = res['월'].map(curr_data)
            
            if prev_y:
                prev_data = df[df['year'] == prev_y].set_index('month')[col]
                res[f'{prev_y}년 평균'] = res['월'].map(prev_data)
                
                # YoY (%) 계산
                y_diff = res[f'{curr_y}년 평균'] - res[f'{prev_y}년 평균']
                y_pct = (y_diff / res[f'{prev_y}년 평균']) * 100
                res['전년비(YoY)'] = [f"{d:+.2f}({p:+.1f}%)" if pd.notnull(d) and pd.notnull(p) else "-" for d, p in zip(y_diff, y_pct)]
            
            # MoM (%) 계산 (통합 시계열 차이 활용)
            df_full = df.sort_values('ym')
            df_full['diff_val'] = df_full[col].diff()
            
            mom_map = df_full[df_full['year'] == curr_y].set_index('month')
            m_diffs = mom_map['diff_val']
            prev_month_vals = mom_map[col] - m_diffs
            m_pcts = (m_diffs / prev_month_vals) * 100
            
            res['전월비(MoM)'] = [f"{d:+.2f}({p:+.1f}%)" if pd.notnull(d) and v > 0 else "-" for d, p, v in zip(res['월'].map(m_diffs), res['월'].map(m_pcts), res['월'].map(prev_month_vals))]
            
            # 데이터가 있는 월만 출력
            return res[res[f'{curr_y}년 평균'].notnull()].reset_index(drop=True)

        st.divider(); st.subheader(f"📅 연도별 병렬 분석 리포트 ({m_mean['year'].max()}년 기준)")
        rc1, rc2 = st.columns(2)
        with rc1:
            st.write("#### 💵 USD 환율 분석")
            usd_res = build_perfect_report(m_mean, 'usd')
            if not usd_res.empty: st.table(usd_res.style.format({'월':'{:.0f}월', f'{m_mean["year"].max()}년 평균':'{:,.2f}'}, na_rep="-"))
        with rc2:
            st.write("#### 💴 CNY 환율 분석")
            cny_res = build_perfect_report(m_mean, 'cny')
            if not cny_res.empty: st.table(cny_res.style.format({'월':'{:.0f}월', f'{m_mean["year"].max()}년 평균':'{:,.2f}'}, na_rep="-"))
    else:
        st.warning("환율 데이터가 없습니다. 상단에서 CSV를 업로드하세요.")