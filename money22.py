...
def run_backup():
    ...
    db_file = 'finance_v27_final.db'
    backup_file = f"backups/backup_{today_str}.db"
    if os.path.exists(db_file) and not os.path.exists(backup_file):
        shutil.copy2(db_file, backup_file)

# 2. 페이지 설정
st.set_page_config(page_title="자금 관리 v27", layout="wide", page_icon="💰")
run_backup()

# 3. DB 연결 및 테이블 초기화 (한글 컬럼명 고정)
def get_db_connection():
    conn = sqlite3.connect('finance_v27_final.db', check_same_thread=False)
    c = conn.cursor()
    # 거래처 마스터
...
# --- 5. 이카운트 엑셀 정밀 분석 및 자동 저장 ---
def process_ecount_auto(file):
    try:
        df_raw = pd.read_excel(file, header=None)
        
        # [1] 전표번호 및 발주일 추출 (20260304-1 -> 2026-03-04)
        raw_oid = str(df_raw.iloc[0, 0]).split(":")[-1].strip() if ":" in str(df_raw.iloc[0,0]) else "00000000-0"
        oid = raw_oid
        odate = f"{oid[:4]}-{oid[4:6]}-{oid[6:8]}"
        ...
        # [3] 금액 파싱 (한화 A5 vs 외화 F열 마지막)
        total_amt = 0.0
        currency = "한화"
        
        # 외화 판별 (F열 마지막 행 숫자 확인)
        last_f = df_raw.iloc[:, 5].last_valid_index()
        val_f = df_raw.iloc[last_f, 5] if last_f is not None else None
        
        if isinstance(val_f, (int, float)) and val_f > 100: # 외화 합계 가정
            total_amt = float(val_f)
            currency = "외화(USD/CNY)"
        else:
            # 한화 판별 (A5셀: 금액 : 32,217,900)
...
# --- 6. 메인 UI ---
tabs = st.tabs(["📝 입금 입력", "📂 입금 엑셀 업로드", "📥 발주서 등록(ERP)", "🔍 상세조회 및 필터", "⚙️ 거래처 관리"])
...
with tabs[0]:
    ...
    with st.form("pay_input_v27", clear_on_submit=True):
        ...
        if st.form_submit_button("입금 내역 저장"):
            if p_vendor == "선택": st.error("거래처를 먼저 선택하세요.")
            else:
...
# --- Tab 3: 발주서 등록 (ERP 자동화) ---
with tabs[2]:
    st.header("📥 이카운트 발주서 자동 등록")
    st.info("파일을 올리면 전표번호, 날짜, 금액을 분석하여 즉시 리스트에 추가합니다.")
    o_file = st.file_uploader("이카운트 엑셀(.xlsx)", type=['xlsx'])
    if o_file:
        success, res = process_ecount_auto(o_file)
    ...
    if not o_df.empty:
        st.dataframe(o_df.sort_values('발주일', ascending=False).style.apply(style_closed_rows, axis=1), use_container_width=True)
...
with tabs[3]:
    ...
    if not p_data.empty:
        ...
        if f_m: df_f = df_f[df_f['입금월'].isin(f_m)]
        ...
        if not o_data.empty:
            target_oid = st.selectbox("마감할 발주번호 선택", o_data[o_data['마감여부']==0]['발주번호'].unique() if not o_data.empty else ["없음"])
...
with tabs[4]:
    ...
    with c_v1:
        with st.form("v_reg_v27", clear_on_submit=True):
    ...
    with c_v2:
        ...
        v_file = st.file_uploader("거래처 CSV 업로드", type=['csv'], key="v_csv")
        if v_file:
