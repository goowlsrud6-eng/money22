...
st.set_page_config(page_title="자금 관리 v77", layout="wide", page_icon="💰")
run_backup()

@st.cache_resource
def get_db_connection():
    conn = sqlite3.connect('finance_final_v77.db', check_same_thread=False)
    c = conn.cursor()
...
def process_ecount_v77(file):
    try:
        ...
        if match.empty:
            add_alert(f"❌ 등록 거절: '{vendor_raw}'은(는) 미등록 업체입니다. 거래처 관리에서 등록 후 다시 [등록 실행]을 눌러주세요.")
            return False
...
if st.session_state.alert_msg:
    ...
    if st.button("닫기"): 
        clear_alerts()
        # st.rerun()을 하지 않음으로써 현재 탭/입력값을 유지함
...
with tabs[0]:
    ...
    with st.form("p_manual_v77", clear_on_submit=True):
        c1, c2 = st.columns(2)
...
with tabs[2]:
    ...
    with c1:
        ...
        # 실패 시 파일을 그대로 두기 위해 key를 동적으로 바꾸지 않음 (성공 시에만 up_key 증가)
        of_list = st.file_uploader("발주서(xlsx) 선택", type=['xlsx'], accept_multiple_files=True)
        if of_list and st.button("🚀 모든 발주서 일괄 등록 실행"):
            clear_alerts()
            all_success = True
            scnt = 0
            for of in of_list:
                if process_ecount_v77(of): scnt += 1
                else: all_success = False
            
            if scnt > 0: st.success(f"✅ {scnt}건 등록 완료!")
            if all_success: # 모두 성공했을 때만 파일 목록 비우기
                st.session_state.up_key += 1
                st.rerun()
    with c2:
        st.subheader("✍️ 수기 등록")
        v_list_m = pd.read_sql("SELECT 거래처명 FROM vendors", conn)
        with st.form("o_manual_v77", clear_on_submit=True):
            c_m1, c_m2 = st.columns(2)
            mi = c_m1.text_input("발주번호")
            mt_order = c_m2.text_input("발주차수")
            md = st.date_input("발주일")
            mv = st.selectbox("거래처 선택", ["선택"] + list(v_list_m['거래처명']))
            mp = st.text_input("상품명")
            c_m3, c_m4 = st.columns(2)
            mt = c_m3.number_input("발주금액", format="%.2f")
            m_cur = c_m4.selectbox("통화", ["한화", "USD", "CNY"]) # 요청하신 통화 추가
    ...
    if not o_data.empty:
        ...
        ev_o = st.data_editor(o_display.sort_values('발주일', ascending=False), hide_index=True, use_container_width=True, disabled=["발주번호"])
        if st.button("💾 발주 정보 및 입금내역 소급 저장"):
            ...
            conn.commit(); st.success("✅ 동기화 업데이트 완료!"); st.rerun()
...
with tabs[3]:
    ...
    if not p_all.empty:
        ...
        cat_sum['잔액합계'] = cat_sum['실입금액'] + cat_sum['선급금액']
...
with tabs[4]:
    ...
    with cl:
        with st.form("v_reg_v77", clear_on_submit=True):
            st.subheader("➕ 수기 등록")
    ...
    if not v_data_m.empty:
        st.subheader("🏢 거래처 리스트 (이름 수정 시 전체 자동 소급 업데이트)")
