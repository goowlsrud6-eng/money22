...
# 2. 페이지 설정 및 DB 연결
st.set_page_config(page_title="자금 관리 v49", layout="wide", page_icon="💰")
run_backup()

@st.cache_resource
def get_db_connection():
    conn = sqlite3.connect('finance_v49_final.db', check_same_thread=False)
    c = conn.cursor()
    # 거래처 마스터
...
with tabs[0]:
    ...
    with st.form("p_manual_v49", clear_on_submit=True):
...
with tabs[1]:
    ...
    st.download_button("📥 입금 양식 다운로드", p_tmp.to_csv(index=False).encode('utf-8-sig'), "payment_temp_v49.csv")
    f_p = st.file_uploader("입금 CSV 선택", type=['csv'], key="p_csv_v49")
    if f_p and st.button("🚀 입금 데이터 저장"):
        try:
            ...
            for _, r in df_p.iterrows():
                ...
                bk, ac, hd = (vi.iloc[0]['은행'], v_i.iloc[0]['계좌번호'], v_i.iloc[0]['예금주']) if not vi.empty else ("","","")
...
with tabs[2]:
    ...
    with c1:
        ...
        of = st.file_uploader("발주서(.xlsx) 업로드", type=['xlsx'], key="of_v49")
        if of: 
            s, r = process_ecount_v49(of)
            if s: st.success(f"등록 성공: {r}")
            else: st.error(f"오류: {r}")
    with c2:
        st.subheader("✍️ 2. 발주서 수기 등록")
        v_l = pd.read_sql("SELECT 거래처명 FROM vendors", conn)
        with st.form("o_manual_v49", clear_on_submit=True):
...
with tabs[4]:
    ...
    with cv1:
        st.subheader("➕ 개별 등록")
        with st.form("v_reg_v49", clear_on_submit=True):
    ...
    with cv2:
        ...
        st.download_button("📥 거래처 양식 다운로드", v_tmp.to_csv(index=False).encode('utf-8-sig'), "vendor_template_v49.csv")
        vf = st.file_uploader("거래처 CSV 업로드", type=['csv'], key="v_csv_v49")
