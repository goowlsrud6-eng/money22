...
def run_backup():
    ...
    if not os.path.exists('backups'):
        os.makedirs('backups')
    db_file = 'finance_final_v119.db'
    today_str = datetime.now().strftime('%Y%m%d')
    backup_file = f"backups/backup_{today_str}.db"
    if os.path.exists(db_file) and not os.path.exists(backup_file):
        shutil.copy2(db_file, backup_file)

st.set_page_config(page_title="자금 관리 시스템 v119", layout="wide", page_icon="💰")
run_backup()

@st.cache_resource
def get_db_connection():
    """데이터베이스 테이블 스키마 정의 (단 한 줄도 생략 없음)"""
    conn = sqlite3.connect('finance_final_v119.db', check_same_thread=False)
    c = conn.cursor()
    # [1] 거래처 마스터
...
with tabs[0]:
    ...
    with st.form("pay_manual_v119", clear_on_submit=True):
        c1, c2 = st.columns(2)
...
with tabs[2]:
    ...
    with c_o1:
        ...
        if of_list and st.button("🚀 일괄 등록 실행"):
            for of in of_list: process_ecount_v119(of)
...
with tabs[3]:
    ...
    if not p_all.empty:
        ...
        st.divider(); st.subheader("📑 상세 리스트 편집 및 삭제")
        
        # [복구 4] 입금 상세 리스트에도 '발주차수' 매핑
        fil_p_merged = pd.merge(fil_p, o_all[['발주번호', '발주차수']], on='발주번호', how='left')
        cols = list(fil_p_merged.columns)
        cols.remove('dt')
        cols.remove('발주차수')
        idx = cols.index('발주번호')
        cols.insert(idx+1, '발주차수') # 발주번호 바로 옆에 배치
        ...
        with eb2:
            with st.form("delete_form_v119", clear_on_submit=True):
...
with tabs[4]:
    ...
    with cv1:
        ...
        with st.form("vn_reg_v119", clear_on_submit=True):
