...
# 2. 페이지 설정 및 DB 연결
st.set_page_config(page_title="자금 관리 v42", layout="wide", page_icon="💰")
run_backup()

@st.cache_resource
def get_db_connection():
    conn = sqlite3.connect('finance_v42_final.db', check_same_thread=False)
    c = conn.cursor()
    # 거래처 마스터
...
# --- 3. 이카운트 정밀 분석 함수 (A2번호, 국내B7/해외C7품목) ---
def process_ecount_v42(file):
    try:
...
with tabs[0]:
    ...
    with st.form("p_manual_v42", clear_on_submit=True):
        ...
        p_am, p_pre, p_cur = c4.number_input("실입금액"), c5.number_input("선급금액"), c6.selectbox("통화", CURRENCIES)
        ...
        if st.form_submit_button("입금 내역 저장"):
            if p_v != "선택":
                ...
                conn.cursor().execute("INSERT INTO payments (발주번호, 입금일, 유형, 거래처명, 상품명, 통화, 실입금액, 선급금액, 메모, 한화환산액, 은행, 계좌번호, 예금주) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
                                     (sel_oid if sel_oid != "없음" else None, p_d.strftime("%Y-%m-%d"), p_cat, p_v, "수기입력", p_cur, p_am, p_pre, p_note, (p_am+p_pre)*rate, vi['은행'], vi['계좌번호'], vi['예금주']))
                conn.commit(); st.success("저장 완료!"); st.rerun()

# --- Tab 2: 입금 엑셀 업로드 (에러 보정 강화) ---
with tabs[1]:
    ...
    st.download_button("📥 통합 양식(CSV) 다운로드", tmp.to_csv(index=False).encode('utf-8-sig'), "payment_v42.csv")
    
    file = st.file_uploader("CSV 업로드", type=['csv'], key="p_csv_v42")
    if file and st.button("🚀 데이터 분석 및 일괄 저장"):
        try:
        ...
        except Exception as e: st.error(f"파일 처리 중 에러: {e}")
...
with tabs[2]:
    ...
    with c1:
        ...
        o_f = st.file_uploader("이카운트 엑셀(.xlsx)", type=['xlsx'], key="o_f_v42")
        if o_f: s, r = process_ecount_v42(o_f); st.success(f"등록 성공: {r}") if s else st.error(f"오류: {r}")
    with c2:
        st.subheader("✍️ 2. 발주서 수기 등록")
        with st.form("o_manual_v42", clear_on_submit=True):
...
with tabs[4]:
    ...
    with cv1:
        st.subheader("➕ 개별 등록")
        with st.form("v_reg_v42", clear_on_submit=True):
    ...
    with cv2:
        ...
        v_file = st.file_uploader("거래처 CSV", type=['csv'], key="v_csv_v42")
