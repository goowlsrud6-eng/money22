...
with tabs[1]:
    ...
    # 1. 데이터 로드 (Supabase 실시간 호출)
    v_master = get_supabase_data("vendors")
    o_data = get_supabase_data("orders")
    
    c1, c2 = st.columns([1, 1.8]) 
    
    # --- 왼쪽: 발주 등록 섹션 (원본 기능 100% 복구) ---
    with c1:
        ...
        with st.form("manual_ord_form_v_final_complete", clear_on_submit=True):
            ...
            m_vn = st.selectbox("거래처 선택", vn_list, help="거래처 마스터에서 선택하세요.")
            
            m_prod = st.text_input("상품명", help="상품명을 입력하세요.")
            
            col_m1, col_m2 = st.columns(2)
            m_amt = col_m1.number_input("발주총액", format="%.2f", step=100.0, help="금액을 입력하세요.")
            ...
            if st.form_submit_button("➕ 발주 저장", use_container_width=True):
                if m_oid and m_vn != "선택":
                    ...
                    st.success(f"발주 {m_oid} 등록 완료!"); st.rerun()
                else:
                    st.error("필수 항목을 입력해 주세요.")

    # --- 오른쪽: 발주 목록 및 마감 관리 섹션 (음영 해결 핵심) ---
    with c2:
        ...
        if not o_data.empty:
            ...
            # [음영 해결의 핵심] !important 강제 지정 및 텍스트 취소선 제거
            def style_row_v136_perfect(row):
                if row['마감여부'] == 1:
                    # 모든 열에 배경색(#f0f2f6)과 글자색(#a1a1a1)을 강제 주입
                    return [
                    ...
                    # 마감 해제 시 배경색을 흰색으로 강제 주입하여 즉시 복구
                    return [
            ...
            # 데이터 에디터 (상태 변화를 즉시 반영하기 위해 데이터 해시를 key에 포함)
            ev_o = st.data_editor(
                disp_o.style.apply(style_row_v136_perfect, axis=1), 
                hide_index=True, 
                use_container_width=True,
                key=f"ord_edit_sync_v{hash(str(disp_o))}", # 데이터 변경 시 에디터 강제 리프레시
                column_config={
            ...
            if st.button("💾 수정 내용 저장 및 입금내역 소급 적용", use_container_width=True):
                try:
                    ...
                    st.success("✅ 모든 정보가 저장 및 소급 적용되었습니다.")
                    st.rerun()
                except Exception as e:
