...
st.set_page_config(page_title="자금 관리 v80", layout="wide", page_icon="💰")
run_backup()

@st.cache_resource
def get_db_connection():
    conn = sqlite3.connect('finance_final_v80.db', check_same_thread=False)
    c = conn.cursor()
    # 거래처 마스터
...
# --- 4. ERP 발주서 분석 로직 (미등록 차단 및 탭 유지형 알람) ---
def process_ecount_v80(file):
    try:
        ...
        if match.empty:
            # 실패 시 에러 메시지만 반환 (화면 튕김 방지)
            return False, f"⚠️ '{vendor_raw}'은(는) 미등록 업체입니다. [거래처 관리]에서 먼저 등록하세요."
...
with tabs[0]:
    ...
    with st.form("p_man_v80", clear_on_submit=True):
        c1, c2 = st.columns(2)
...
with tabs[2]:
    ...
    with c1:
        ...
        if of_list and st.button("🚀 모든 발주서 일괄 등록 실행"):
            ...
            err_msgs = []
            for of in of_list:
                res, msg = process_ecount_v80(of)
                if res: scnt += 1
                else: err_msgs.append(msg)
            
            # 오류가 있을 경우 노란색 경고창으로 표시 (튕기지 않음)
            if err_msgs:
                for em in err_msgs: st.warning(em)
            
            if scnt > 0: 
                st.success(f"✅ {scnt}건 등록 성공!")
                if not err_msgs: # 모든 파일이 성공했을 때만 목록 비우기
    ...
    with c2:
        ...
        with st.form("o_man_v80", clear_on_submit=True):
            c_m1, c_m2 = st.columns(2)
            mi, mt_order = c_m1.text_input("발주번호"), c_m2.text_input("발주차수")
            md, mv = st.date_input("발주일"), st.selectbox("거래처 선택", ["선택"] + list(v_list_manual['거래처명']))
    ...
    if not o_data.empty:
        st.subheader("📄 등록된 발주서 현황 (차수 수기 입력 및 자동 연동)")
        o_disp = o_data[['발주번호', '발주차수', '거래처명', '상품명', '유형', '통화', '발주총액', '마감여부', '발주일']]
        ev_o = st.data_editor(o_disp.sort_values('발주일', ascending=False), hide_index=True, use_container_width=True, disabled=["발주번호"], key="o_edit_v80")
        if st.button("💾 발주 정보 및 입금내역 소급 저장"):
            for idx, r in ev_o.iterrows():
                ...
                # 발주번호 기준 입금 상세내역(payments) 연동 동기화
...
with tabs[4]:
    ...
    with cl:
        with st.form("v_reg_v80", clear_on_submit=True):
            st.subheader("➕ 수기 등록")
            vn, vt = st.text_input("거래처명"), st.selectbox("유형", CATEGORIES)
            vc1, vc2, vc3 = st.columns(3)
            vb, va, vh = vc1.text_input("은행"), vc2.text_input("계좌번호"), vc3.text_input("예금주")
            if st.form_submit_button("✅ 저장"):
