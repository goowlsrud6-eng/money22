...
# 2. 페이지 설정 및 DB 연결
st.set_page_config(page_title="자금 관리 v52", layout="wide", page_icon="💰")
run_backup()

@st.cache_resource
def get_db_connection():
    conn = sqlite3.connect('finance_v52_final.db', check_same_thread=False)
    c = conn.cursor()
...
with tabs[0]: # 수기 입력
    ...
    with st.form("p_manual_v52", clear_on_submit=True):
...
with tabs[1]: # 입금 엑셀 업로드 (클리닝 강화)
    st.header("📂 통합 입금 엑셀 업로드")
    p_tmp = pd.DataFrame(columns=["발주번호", "거래처", "유형", "상품명", "입금일", "실입금액", "선급금액", "송금사유"])
    st.download_button("📥 입금 양식 다운로드", p_tmp.to_csv(index=False).encode('utf-8-sig'), "pay_temp_v52.csv")
    f_p = st.file_uploader("CSV 선택", type=['csv'], key="p_csv_v52")
        ...
        try:
            ...
            for _, r in df_p.iterrows():
                ...
                if oid != "nan" and not o_df[o_df['발주번호'] == oid].empty:
                    ...
                    vn, pc, pp, cur = vn, str(r['유형']), str(r['상품명']), "한화"
...
with tabs[2]: # 발주서 등록
    ...
    c1, c2 = st.columns(2)
    with c1:
        of = st.file_uploader("엑셀 업로드", type=['xlsx'], key="of_v52")
        if of: s, r = process_ecount_v52(of); st.success(f"성공: {r}") if s else st.error(f"오류: {r}")
    with c2:
        with st.form("o_manual_v52", clear_on_submit=True):
...
with tabs[3]: # 상세내역 및 정산 (삭제 기능 포함)
...
with tabs[4]: # 거래처 관리
    ...
    with cv1:
        with st.form("v_reg_v52", clear_on_submit=True):
    ...
    with cv2:
        ...
        st.download_button("📥 거래처 양식 다운로드", v_tmp.to_csv(index=False).encode('utf-8-sig'), "vendor_template_v52.csv")
        vf = st.file_uploader("거래처 CSV 업로드", type=['csv'], key="v_csv_v52")
