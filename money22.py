...
# --- 1. 백업 및 데이터베이스 설정 ---
def run_backup():
    """안전한 데이터 보관을 위해 매일 백업 생성"""
    if not os.path.exists('backups'):
        os.makedirs('backups')
    db_file = 'finance_final_v109.db'
...
# 레이아웃 설정
st.set_page_config(page_title="자금 관리 시스템 v109", layout="wide", page_icon="💰")
run_backup()

@st.cache_resource
def get_db_connection():
    """테이블 스키마 정의 (단 하나도 생략 없음)"""
    conn = sqlite3.connect('finance_final_v109.db', check_same_thread=False)
    c = conn.cursor()
    # 거래처 마스터
...
with tabs[0]:
    ...
    with st.form("manual_pay_v109", clear_on_submit=True):
        c1, c2 = st.columns(2)
...
with tabs[1]:
    ...
    st.download_button(label="📥 입금 업로드 샘플 다운로드", data=pay_template.to_csv(index=False).encode('utf-8-sig'), file_name='payment_template.csv', mime='text/csv')
...
with tabs[2]:
    ...
    with col1:
        ...
        if of_list and st.button("🚀 모든 발주서 일괄 등록"):
            for of in of_list: process_ecount_v109(of)
    ...
    if not o_data.empty:
        ...
        show_c = st.checkbox("마감된 발주 포함", value=False)
...
with tabs[3]:
    ...
    if not p_all.empty:
        ...
        st.divider(); st.subheader("📊 정산 및 미수금 현황")
...
with tabs[5]:
    ...
    if not ex_db.empty:
        ...
        # 월별 평균 가공
        monthly_mean = ex_db.groupby('연월').agg({'usd': lambda x: x[x > 0].mean(), 'cny': lambda x: x[x > 0].mean()}).reset_index().fillna(0)
        ...
        def get_final_report(df, col_name):
            available_years = sorted(df['year'].unique(), reverse=True)
            pivot_data = df.pivot(index='month', columns='year', values=col_name).sort_index()
            curr_year = available_years[0]
            prev_year = available_years[1] if len(available_years) > 1 else None
            
            report = pd.DataFrame(index=pivot_data.index)
            if prev_year: report[f'{prev_year}년 평균'] = pivot_data[prev_year]
            report[f'{curr_year}년 평균'] = pivot_data[curr_year]
        ...
        with rc1:
            st.write("#### 💵 USD 환율 분석")
            usd_res = get_final_report(monthly_mean, 'usd')
            if not usd_res.empty: st.table(usd_res.style.format({'month':'{:.0f}월', f'{available_years[0]}년 평균':'{:,.2f}'}, na_rep="-"))
        with rc2:
            st.write("#### 💴 CNY 환율 분석")
            cny_res = get_final_report(monthly_mean, 'cny')
            if not cny_res.empty: st.table(cny_res.style.format({'month':'{:.0f}월', f'{available_years[0]}년 평균':'{:,.2f}'}, na_rep="-"))
    else:
        st.warning("데이터가 없습니다. CSV 파일을 업로드해 주세요.")