...
st.set_page_config(page_title="자금 관리 v65", layout="wide", page_icon="💰")
run_backup()

@st.cache_resource
def get_db_connection():
    conn = sqlite3.connect('finance_v65_final.db', check_same_thread=False)
    c = conn.cursor()
    # 거래처 마스터
...
# --- 3. ERP 발주서 분석 로직 (알람 기능 포함) ---
def process_ecount_v65(file):
    try:
        ...
        if v_check.empty:
            st.warning(f"⚠️ '{vendor}'는 미등록 거래처입니다. 발주서 등록 전 [거래처 관리]에서 등록을 권장합니다.")
            v_type = "사입"
        else:
            v_type = v_check.iloc[0]['기본유형']
        ...
        if st.form_submit_button("✅ 저장"):
            if p_vn != "선택":
                ...
                conn.execute("INSERT INTO payments (발주번호, 입금일, 유형, 거래처명, 상품명, 통화, 실입금액, 선급금액, 메모, 한화환산액, 은행, 계좌번호, 예금주) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
                             (p_oid if p_oid != "없음" else None, p_date.strftime("%Y-%m-%d"), p_ct, p_vn, p_pr, p_curr, p_dep, p_pre, p_note, (p_dep+p_pre)*rate, vi['은행'], vi['계좌번호'], vi['예금주']))
...
with tabs[1]:
    ...
    st.download_button("📥 입금 양식 다운로드", p_template.to_csv(index=False).encode('utf-8-sig'), "payment_template.csv")
    f_p = st.file_uploader("입금 CSV 선택", type=['csv'], key="up_pay_csv")
...
# [Tab 2] 발주서 등록 및 관리 (수정 기능 포함)
with tabs[2]:
    st.header("📥 발주서 등록 및 관리")
    c1, c2 = st.columns(2)
    with c1:
        st.subheader("⚡ 엑셀 일괄 등록")
        of_list = st.file_uploader("발주서(.xlsx) 다중 선택", type=['xlsx'], accept_multiple_files=True)
        if of_list and st.button("🚀 모든 파일 등록"):
            for of in of_list: process_ecount_v65(of)
            st.rerun()
    with c2:
        ...
        with st.form("o_man_form"):
            ...
            mp, mt = st.text_input("상품명"), st.number_input("금액")
    ...
    if not o_data.empty:
        st.subheader("📄 발주서 리스트 (더블클릭 수정 가능)")
        edited_o = st.data_editor(o_data.sort_values('발주일', ascending=False), hide_index=True, use_container_width=True, disabled=["발주번호"], key="o_edit_v65")
        if st.button("💾 발주 리스트 수정사항 저장"):
            ...
            conn.commit(); st.success("발주 리스트 업데이트 완료"); st.rerun()

# [Tab 3] 상세내역 및 정산 (요청 순서 재배치 + 월별 필터 + 잔액 정렬)
with tabs[3]:
    ...
    if not p_all.empty:
        ...
        st.divider()
        # 2. 상세내역 리스트 (최신순 + 편집)
        st.subheader("📑 상세 입금 내역 리스트 (수정/삭제 가능)")
        p_all_disp = p_all.sort_values('입금일', ascending=False).drop(columns=['입금일_dt'])
        edited_p = st.data_editor(p_all_disp, hide_index=True, use_container_width=True, disabled=["id"], key="p_edit_v65")
...
# [Tab 4] 거래처 관리 (2단 레이아웃 + 동기화 기능)
with tabs[4]:
    st.header("⚙️ 거래처 관리")
    cl, cr = st.columns([1.2, 0.8])
    with cl:
        with st.form("v_reg_v65", clear_on_submit=True):
            st.subheader("➕ 수기 등록")
    ...
    with cr:
        st.subheader("📂 엑셀 등록")
        v_file = st.file_uploader("거래처 CSV", type=['csv'])
        if v_file and st.button("🚀 업로드"):
    ...
    v_data = pd.read_sql("SELECT * FROM vendors", conn)
    if not v_data.empty:
        st.subheader("🏢 거래처 목록 (수정 시 발주서 동기화)")
        ev = st.data_editor(v_data, hide_index=True, use_container_width=True, key="v_edit_v65")
        if st.button("💾 정보 업데이트 및 모든 유형 동기화"):
            for idx, r in ev.iterrows():
                conn.execute("UPDATE vendors SET 은행=?, 계좌번호=?, 예금주=?, 기본유형=? WHERE 거래처명=?", (r['은행'],r['계좌번호'],r['예금주'],r['기본유형'],r['거래처명']))
                # 거래처 정보가 바뀌면 발주서와 입금내역의 유형도 즉시 갱신
                conn.execute("UPDATE orders SET 유형=? WHERE 거래처명=?", (r['기본유형'], r['거래처명']))
                conn.execute("UPDATE payments SET 유형=? WHERE 거래처명=?", (r['기본유형'], r['거래처명']))
