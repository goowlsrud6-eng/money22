...
def run_backup():
    ...
    db_file = 'finance_v39_final.db'
    backup_file = f"backups/backup_{today_str}.db"
    if os.path.exists(db_file) and not os.path.exists(backup_file):
        shutil.copy2(db_file, backup_file)

# 2. 페이지 설정 및 DB 연결
st.set_page_config(page_title="자금 관리 v39", layout="wide", page_icon="💰")
run_backup()

@st.cache_resource
def get_db_connection():
    conn = sqlite3.connect('finance_v39_final.db', check_same_thread=False)
    c = conn.cursor()
...
# --- 3. 이카운트 정밀 분석 함수 (A2번호, B7/C7품목, F6중국/USD) ---
def process_ecount_v39(file):
    try:
...
with tabs[0]:
    ...
    with st.form("p_manual_v39", clear_on_submit=True):
...
# --- Tab 2: 지능형 입금 엑셀 업로드 ---
with tabs[1]:
    st.header("📂 지능형 입금 엑셀 업로드")
    simple_temp = pd.DataFrame(columns=["발주번호", "입금일", "실입금액", "선급금액", "송금사유"])
    st.download_button("📥 초간단 입금 양식 다운로드", simple_temp.to_csv(index=False).encode('utf-8-sig'), "easy_payment.csv")
    p_file = st.file_uploader("입금 CSV 업로드", type=['csv'], key="p_csv_v39")
    if p_file and st.button("🚀 발주번호 매칭 및 일괄 저장"):
        df_p = pd.read_csv(p_file); o_df = load_table("orders"); v_df = load_table("vendors")
        for _, r in df_p.iterrows():
            oid = str(r['발주번호']).strip(); order_info = o_df[o_df['발주번호'] == oid]
            if not order_info.empty:
                v_n = order_info.iloc[0]['거래처명']; p_cat = order_info.iloc[0]['유형']; p_prod = order_info.iloc[0]['상품명']; p_curr = order_info.iloc[0]['통화']
                v_i = v_df[v_df['거래처명'] == v_n]; bank, acc, holder = (v_i.iloc[0]['은행'], v_i.iloc[0]['계좌번호'], v_i.iloc[0]['예금주']) if not v_i.empty else ("","","")
                rate = 1350.0 if p_curr == "USD" else (190.0 if p_curr == "CNY" else 1.0)
                conn.cursor().execute("INSERT INTO payments (발주번호, 입금일, 유형, 거래처명, 상품명, 통화, 실입금액, 선급금액, 메모, 한화환산액, 은행, 계좌번호, 예금주) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
                                    (oid, r['입금일'], p_cat, v_n, p_prod, p_curr, r['실입금액'], r['선급금액'], r['송금사유'], (float(r['실입금액'])+float(r['선급금액']))*rate, bank, acc, holder))
        conn.commit(); st.success("매칭 저장 완료!"); st.rerun()
...
with tabs[2]:
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.subheader("⚡ 1. 엑셀 즉시 등록")
        o_f = st.file_uploader("이카운트 엑셀(.xlsx)", type=['xlsx'], key="o_up_v39")
        if o_f: 
            s, res = process_ecount_v39(o_f)
            if s: st.success(f"등록 성공: {res}")
            else: st.error(f"실패: {res}")
    with c2:
        st.subheader("✍️ 2. 발주서 수기 등록")
        with st.form("o_manual_v39", clear_on_submit=True):
...
with tabs[4]:
    ...
    with cv1:
        st.subheader("➕ 개별 등록")
        with st.form("v_reg_v39", clear_on_submit=True):
    ...
    with cv2:
        ...
        v_f = st.file_uploader("거래처 CSV", type=['csv'], key="v_csv_v39")
