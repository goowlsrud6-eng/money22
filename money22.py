...
st.set_page_config(page_title="자금 관리 v76", layout="wide", page_icon="💰")
run_backup()

@st.cache_resource
def get_db_connection():
    conn = sqlite3.connect('finance_final_v76.db', check_same_thread=False)
    c = conn.cursor()
    # 거래처 마스터 (기본키: 거래처명)
...
def process_ecount_v76(file):
    try:
        ...
        if match.empty:
            add_alert(f"❌ 등록 거부: '{vendor_raw}'은(는) 미등록 업체입니다. [거래처 관리]에서 먼저 등록하세요.")
            return False
...
# [Tab 2] 발주서 등록 및 관리 (발주-상세내역 자동 연동 수정)
with tabs[2]:
    ...
    with c1:
        ...
        if of_list and st.button("🚀 모든 발주서 일괄 등록 실행"):
            clear_alerts()
            scnt = sum(1 for of in of_list if process_ecount_v76(of))
    ...
    if not o_data.empty:
        ...
        # 발주일 대신 차수를 앞으로 배치하여 표시
        o_display = o_data[['발주번호', '발주차수', '거래처명', '상품명', '유형', '통화', '발주총액', '마감여부', '발주일']]
        ev_o = st.data_editor(o_display.sort_values('발주일', ascending=False), hide_index=True, use_container_width=True, disabled=["발주번호"], key="o_edit_v76")
        if st.button("💾 발주 정보 및 입금내역 소급 저장"):
            for idx, r in ev_o.iterrows():
                ...
                # 2. ★ 발주번호를 기준으로 상세내역(payments)의 거래처명과 유형까지 동시 업데이트 ★
                conn.execute("UPDATE payments SET 거래처명=?, 유형=? WHERE 발주번호=?", (r['거래처명'], r['유형'], r['발주번호']))
            conn.commit(); st.success("✅ 발주서 및 연동된 모든 상세 내역이 업데이트되었습니다."); st.rerun()
...
with tabs[3]:
    ...
    if not p_all.empty:
        ...
        if st.button("🗑️ 해당 ID 행 삭제"): conn.execute(f"DELETE FROM payments WHERE id={did}"); conn.commit(); st.rerun()
