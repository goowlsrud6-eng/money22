...
st.set_page_config(page_title="자금 관리 v78", layout="wide", page_icon="💰")
run_backup()

@st.cache_resource
def get_db_connection():
    conn = sqlite3.connect('finance_final_v78.db', check_same_thread=False)
    c = conn.cursor()
...
def process_ecount_v78(file):
    try:
        ...
        if match.empty:
            add_alert(f"❌ 등록 실패: '{vendor_raw}'은(는) 미등록 업체입니다. [거래처 관리]에서 등록 후 다시 시도하세요.")
            return False
        ...
        if curr != "한화":
            ...
            total = float(df.iloc[l_idx, 5]) if l_idx is not None else 0.0
        else:
...
with tabs[0]:
    ...
    with st.form("p_manual_v78", clear_on_submit=True):
        c1, c2 = st.columns(2)
...
with tabs[1]:
    ...
    if f_p and st.button("🚀 입금 데이터 일괄 저장"):
        try:
            ...
            conn.commit(); st.success(f"✅ 총 {len(df_p)}건 저장 완료!")
            st.session_state.pay_up_key += 1 # 파일 비우기
            st.rerun()
        except Exception as e: st.error(f"입금 엑셀 오류: {e}")

# [Tab 2] 발주서 등록 (성공 시 파일 삭제 / 실패 시 유지)
with tabs[2]:
    ...
    with c1:
        ...
        if of_list and st.button("🚀 모든 발주서 일괄 등록 실행"):
            ...
            scnt = 0
            for of in of_list:
                if process_ecount_v78(of): scnt += 1
                else: all_success = False
            
            if scnt > 0: st.success(f"✅ {scnt}건 등록 성공!")
            if all_success: # ★ 100% 모두 성공했을 때만 파일 목록 비우기
                st.session_state.order_up_key += 1
                st.rerun()
            else: st.warning("일부 파일 등록 실패. 미등록 업체명을 확인 후 다시 [등록 실행]을 눌러주세요.")
    with c2:
        st.subheader("✍️ 수기 등록")
        v_list_m = pd.read_sql("SELECT 거래처명 FROM vendors", conn)
        with st.form("o_manual_v78", clear_on_submit=True):
    ...
    if not o_data.empty:
        ...
        ev_o = st.data_editor(o_display.sort_values('발주일', ascending=False), hide_index=True, use_container_width=True, disabled=["발주번호"], key="o_edit_v78")
...
with tabs[4]:
    ...
    with cl:
        with st.form("v_reg_v78", clear_on_submit=True):
            st.subheader("➕ 수기 등록")
