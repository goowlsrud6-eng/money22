...
# 2. 페이지 설정 및 DB 연결
st.set_page_config(page_title="자금 관리 v48", layout="wide", page_icon="💰")
run_backup()

@st.cache_resource
def get_db_connection():
    conn = sqlite3.connect('finance_v48_final.db', check_same_thread=False)
    c = conn.cursor()
    # 거래처 마스터
...
with tabs[0]:
    ...
    with st.form("p_manual_v48", clear_on_submit=True):
...
with tabs[1]:
    ...
    st.download_button("📥 통합 양식(CSV) 다운로드", tmp.to_csv(index=False).encode('utf-8-sig'), "payment_template_v48.csv")
    f_p = st.file_uploader("입금 CSV 선택", type=['csv'], key="p_csv_v48")
...
with tabs[2]:
    ...
    with c1:
        ...
        o_f = st.file_uploader("발주서(.xlsx)", type=['xlsx'], key="o_f_v48")
        if o_f: 
            s, res = process_ecount_v48(o_f)
            if s: st.success(f"등록 성공: {res}")
            else: st.error(f"오류: {res}")
    with c2:
        st.subheader("✍️ 2. 발주서 수기 등록")
        v_l = pd.read_sql("SELECT 거래처명 FROM vendors", conn)
        with st.form("o_manual_v48", clear_on_submit=True):
    ...
    if not o_list.empty:
        o_disp = o_list.copy(); o_disp['발주총액'] = o_disp['발주총액'].apply(format_num)
        st.dataframe(o_disp.sort_values('발주일', ascending=False).style.apply(style_row, axis=1), use_container_width=True, hide_index=True)

# --- Tab 4: 상세내역 및 정산 (인덱스 숨기기 및 잔액 계산) ---
with tabs[3]:
    ...
    if not p_all.empty:
        ...
        c1, c2 = st.columns(2)
        with c1:
            del_id = st.number_input("삭제할 ID(id열 숫자) 입력", min_value=0)
...
with tabs[4]:
    ...
    with cv1:
        with st.form("v_reg_v48", clear_on_submit=True):
    ...
    with cv2:
        v_f = st.file_uploader("거래처 CSV", type=['csv'], key="v_csv_v48")
