...
st.set_page_config(page_title="자금 관리 v72", layout="wide", page_icon="💰")
run_backup()

@st.cache_resource
def get_db_connection():
    conn = sqlite3.connect('finance_final_v72.db', check_same_thread=False)
    c = conn.cursor()
    # 거래처 마스터
...
def process_ecount_v72(file):
    try:
        ...
        if match.empty:
            add_alert(f"❌ 등록 차단: '{vendor_raw}'는 미등록 업체입니다. 먼저 [거래처 관리]에서 등록하세요.")
            return False
        ...
        add_alert(f"❗ 오류({file.name}): {str(e)}"); return False
...
with tabs[0]:
    ...
    with st.form("p_manual_v72", clear_on_submit=True):
        c1, c2 = st.columns(2)
...
with tabs[2]:
    ...
    with c1:
        ...
        of_list = st.file_uploader("xlsx 파일 다중 선택", type=['xlsx'], accept_multiple_files=True, key=f"ord_{st.session_state.up_key}")
        if of_list and st.button("🚀 모든 발주서 일괄 업로드"):
            clear_alerts()
            scnt = sum(1 for of in of_list if process_ecount_v72(of))
            if scnt > 0: st.success(f"✅ {scnt}건 등록 완료!"); st.session_state.up_key += 1; st.rerun()
    with c2:
        st.subheader("✍️ 수기 등록")
        with st.form("o_manual_v72", clear_on_submit=True):
...
with tabs[3]:
    ...
    if not p_all.empty:
        ...
        sy = f_y.selectbox("연도", sorted(p_all['입금일_dt'].dt.year.unique(), reverse=True))
        sm = f_m.selectbox("월", ["전체"] + list(range(1, 13)))
        ...
        st.divider()
        # 2. 상세내역 리스트
        st.subheader("📑 상세 입금 내역 (수정/삭제 가능)")
        ...
        if not o_all.empty:
            ...
            st.table(sum_df[['발주번호', '발주일', '거래처명', '발주총액', '실입금액', '선급금액', '잔액']].style.format('{:,.2f}'))
...
with tabs[4]:
    ...
    with cl:
        with st.form("v_reg_v72", clear_on_submit=True):
            st.subheader("➕ 수기 등록")
    ...
    if not v_d.empty:
        st.subheader("🏢 거래처 리스트 (이름 수정 시 전체 소급 동기화)")
        orig_names = v_d['거래처명'].tolist()
        ev_v = st.data_editor(v_d, hide_index=True, use_container_width=True)
