...
def run_backup():
    ...
    if not os.path.exists('backups'):
        os.makedirs('backups')
    db_file = 'finance_final_v118.db'
    today_str = datetime.now().strftime('%Y%m%d')
    backup_file = f"backups/backup_{today_str}.db"
    if os.path.exists(db_file) and not os.path.exists(backup_file):
        shutil.copy2(db_file, backup_file)

st.set_page_config(page_title="자금 관리 시스템 v118", layout="wide", page_icon="💰")
run_backup()

@st.cache_resource
def get_db_connection():
    """데이터베이스 테이블 스키마 정의 (단 한 줄도 생략 없음)"""
    conn = sqlite3.connect('finance_final_v118.db', check_same_thread=False)
    c = conn.cursor()
    # [1] 거래처 마스터
...
def process_ecount_v118(file):
    """ERP 발주서 분석 엔진 (에러 메시지 반환 보장)"""
    try:
        ...
        if match.empty: 
            return False, f"⚠️ '{vendor_raw}' 미등록 업체입니다. 거래처 관리에서 먼저 등록하세요."
...
with tabs[0]:
    ...
    with st.form("pay_manual_v118", clear_on_submit=True):
        c1, c2 = st.columns(2)
...
# ------------------------------------------
# [Tab 2] 발주서 등록 및 마감 (에러 수집 로직 완벽 복구!!!)
# ------------------------------------------
with tabs[2]:
    ...
    with c_o1:
        ...
        if of_list and st.button("🚀 일괄 등록 실행"):
            ...
            for of in of_list: 
                is_success, err_msg = process_ecount_v118(of)
                if is_success:
                    success_cnt += 1
                else:
            ...
            if success_cnt > 0:
                st.success(f"✅ 총 {success_cnt}건의 발주서 등록 성공!")
...
# ------------------------------------------
# [Tab 3] 상세내역 및 통합 정산 (포맷팅 에러 원상 복구)
# ------------------------------------------
with tabs[3]:
    ...
    if not p_all.empty:
        ...
        if not fil_p.empty:
            ...
            # ★★★ ValueError가 발생했던 부분 v91 원본으로 완전 복구 (숫자 컬럼만 지정) ★★★
...
with tabs[4]:
    ...
    with cv1:
        ...
        with st.form("vn_reg_v118", clear_on_submit=True):
...
with tabs[5]:
    ...
    if not ex_db.empty:
        ...
        # 리포트 생성 로직 (1~12월 뼈대 기반, 1월 전월비 버그 완벽 수정)
        ...
        st.divider()
        st.subheader(f"📅 연도별 병렬 분석 리포트 (YoY & MoM 통합)")
