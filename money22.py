    ...
    if not p_all.empty:
        ...
        def get_conv(row):
            try:
                ...
                return float(row['실입금액']) * rates.get(row['통화'], 0)
            except: return 0
        ...
        def highlight_row(row):
            ...
            return style
        st.dataframe(disp_s.style.apply(highlight_row, axis=1).format('{:,.0f}', subset=['발주총액','실입금액','선급금액','미수잔액']), hide_index=True, use_container_width=True)
