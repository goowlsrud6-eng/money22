...
st.set_page_config(page_title="자금 관리 v67", layout="wide", page_icon="💰")
run_backup()

@st.cache_resource
def get_db_connection():
    conn = sqlite3.connect('finance_final_v67.db', check_same_thread=False)
    c = conn.cursor()
...
def process_ecount_v67(file):
    try:
        ...
        if v_check.empty:
            set_msg(f"⚠️ '{vendor}'는 미등록 거래처입니다. 발주서 등록 전 [거래처 관리]에서 등록하거나 오타를 확인하세요!", "warning")
            v_type = "사입"
        else: v_type = v_check.iloc[0]['기본유형']
        ...
        conn.execute("INSERT OR REPLACE INTO orders VALUES (?,?,?,?,?,?,?,0)", (raw_oid, odate, vendor, prod_n, v_type, curr, total))
...
with tabs[0]:
    ...
    o_act = pd.read_sql("SELECT * FROM orders WHERE 마감여부=0", conn)
    with st.form("p_man_v67", clear_on_submit=True):
        c1, c2 = st.columns(2)
        p_oid = c1.selectbox("🔗 발주번호 연동", ["없음"] + list(o_act['발주번호']) if not o_act.empty else ["없음"])
        p_date = c2.date_input("입금일")
        c3, c4, c5 = st.columns(3)
        p_vn = c3.selectbox("거래처명", ["선택"] + list(v_data['거래처명']) if not v_data.empty else ["선택"])
        p_ct = c4.selectbox("유형", CATEGORIES); p_pr = c5.text_input("상품명")
...
with tabs[1]:
    ...
    f_p = st.file_uploader("CSV 선택", type=['csv'])
    if f_p and st.button("🚀 일괄 저장"):
        try:
...
with tabs[2]:
    ...
    with c1:
        ...
        if of_list and st.button("🚀 모든 파일 일괄 업로드"):
            for of in of_list: process_ecount_v67(of)
            st.rerun()
    with c2:
        with st.form("o_man_v67"):
            st.subheader("✍️ 발주 수기 등록")
    ...
    st.divider()
    st.subheader("📄 발주서 리스트 (수정 가능 / 최신순 정렬)")
...
with tabs[3]:
    ...
    if not p_all.empty:
        ...
        st.divider()
        # 2. 상세내역 리스트
        st.subheader("📑 상세 입금 내역 리스트 (수정/삭제)")
        ...
        if c_del1.button("🗑️ 해당 ID 행 삭제"): conn.execute(f"DELETE FROM payments WHERE id={did}"); conn.commit(); st.rerun()
...
# [Tab 4] 거래처 관리 (2단 레이아웃 + 완전 동기화)
with tabs[4]:
    st.header("⚙️ 거래처 관리")
    cl, cr = st.columns([1.2, 0.8])
    with cl:
        with st.form("v_reg_v67"):
            st.subheader("➕ 수기 등록")
    ...
    v_data = pd.read_sql("SELECT * FROM vendors", conn)
    if not v_data.empty:
        st.subheader("🏢 거래처 목록 (수정 시 이름/유형 전체 자동 동기화)")
        ev_v = st.data_editor(v_data, hide_index=True, use_container_width=True, key="v_ed_v67")
        if st.button("💾 거래처 및 관련 모든 데이터 동기화 저장"):
            orig_names = pd.read_sql("SELECT 거래처명 FROM vendors", conn)['거래처명'].tolist()
            for idx, r in ev_v.iterrows():
                old_name = orig_names[idx] if idx < len(orig_names) else None
                new_name = r['거래처명']
            ...
            conn.commit(); set_msg("✅ 거래처 및 모든 관련 데이터 동기화 완료!", "success"); st.rerun()