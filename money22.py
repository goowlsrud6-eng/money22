...
def run_backup():
    ...
    if not os.path.exists('backups'):
        os.makedirs('backups')
    db_file = 'finance_final_v112.db'
    today_str = datetime.now().strftime('%Y%m%d')
    backup_file = f"backups/backup_{today_str}.db"
    if os.path.exists(db_file) and not os.path.exists(backup_file):
        shutil.copy2(db_file, backup_file)

st.set_page_config(page_title="자금 관리 시스템 v112", layout="wide", page_icon="💰")
run_backup()

@st.cache_resource
def get_db_connection():
    """DB 연결 및 모든 테이블 스키마 생성"""
    conn = sqlite3.connect('finance_final_v112.db', check_same_thread=False)
    c = conn.cursor()
    # 거래처 마스터
...
with tabs[0]:
    ...
    with st.form("manual_pay_form_v112", clear_on_submit=True):
        c1, c2 = st.columns(2)
        p_oid = c1.selectbox("🔗 발주번호 연동 (진행중)", ["없음"] + list(o_active['발주번호']))
...
with tabs[2]:
    ...
    with col1:
        ...
        if of_list and st.button("🚀 모든 발주서 일괄 등록 실행"):
            for of in of_list: process_ecount_v112(of)
    ...
    with col2:
        ...
        with st.form("manual_order_form_v112"):
...
with tabs[3]:
    ...
    if not p_all.empty:
        ...
        st.divider(); st.subheader("📊 발주별 미수금 현황")
...
with tabs[4]:
    ...
    with cv1:
        ...
        with st.form("vendor_reg_form_v112", clear_on_submit=True):
...
with tabs[5]:
    ...
    if not ex_db.empty:
        ...
        def build_full_report(df, col):
            ...
            # 전월비(MoM) - 통합 시계열
            df_sorted = df.sort_values('ym')
            df_sorted['diff'] = df_sorted[col].diff()
            mom_map = df_sorted[df_sorted['year'] == curr_y].set_index('month')
            m_diffs = mom_map['diff']
            prev_vals = mom_map[col] - m_diffs
            m_pcts = (m_diffs / prev_vals) * 100
            res['전월비(MoM)'] = [f"{d:+.2f}({p:+.1f}%)" if pd.notnull(d) and v > 0 else "-" for d, p, v in zip(res['월'].map(m_diffs), res['월'].map(m_pcts), res['월'].map(prev_vals))]
            
            return res[res[f'{curr_y}년 평균'].notnull()].reset_index(drop=True)

        st.divider(); st.subheader("📅 연도별 병렬 분석 리포트")
        rc1, rc2 = st.columns(2)
        with rc1:
            st.write("#### 💵 USD 분석")
            usd_res = build_full_report(m_mean, 'usd')
            if not usd_res.empty: st.table(usd_res.style.format({'월':'{:.0f}월'}, na_rep="-"))
        with rc2:
            st.write("#### 💴 CNY 분석")
            cny_res = build_full_report(m_mean, 'cny')
            if not cny_res.empty: st.table(cny_res.style.format({'월':'{:.0f}월'}, na_rep="-"))
    else:
